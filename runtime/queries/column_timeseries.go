...
func getCoalesceStatementsMeasures(dialect drivers.Dialect, measures []*runtimev1.ColumnTimeSeriesRequest_BasicMeasure) string {
	var result string
	for i, measure := range measures {
		escapedName := dialect.EscapeIdentifier(measure.SqlName)
		result += fmt.Sprintf(`series.%[1]s as %[1]s`, escapedName)
		if i < len(measures)-1 {
			result += ", "
		}
	}
...
func getCoalesceStatementsMeasuresLast(dialect drivers.Dialect, measures []*runtimev1.ColumnTimeSeriesRequest_BasicMeasure) string {
	...
	for i, measure := range measures {
		...
		case drivers.DialectDuckDB:
			...
			result += fmt.Sprintf(` `+lastValue(dialect)+`(%[1]s ORDER BY %[1]s NULLS FIRST) as %[1]s`, escapedName)
		default:
			result += fmt.Sprintf(` `+lastValue(dialect)+`(%[1]s) as %[1]s`, escapedName)
		}
		if i < len(measures)-1 {
			result += ", "
		}
